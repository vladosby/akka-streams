...
//  illegalSource.to(Sink.foreach(println)).run()
  ...
  val source1 = Source(names)
  val filterFlow = Flow[String].filter(_.length > 5)
  val reduceFlow = Flow[String].take(2)
  val sink1 = Sink.foreach[String](println)

  val resultGraph = source1.via(filterFlow).via(reduceFlow).to(sink1)
  resultGraph.run()
  source1.filter(_.length > 5).take(2).runForeach(println)
